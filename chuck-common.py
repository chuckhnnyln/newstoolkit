...
def GetFileDialog(AppTitle):
    global TitleFont, Font, Pad
    ChooseTargetLayout = [  [sg.Text('Please choose target file.')],
                            [sg.Input(size = (25,1), key = '_FILEPATH_'), sg.FileBrowse(button_text = 'Browse', enable_events = True)],
                            [sg.Button('Next', pad = (Pad)), sg.Button('Quit', pad = (Pad))]    ]
    ChooserWindow = sg.Window(AppTitle, ChooseTargetLayout, font=(Font))
    while True:
        event, values = ChooserWindow.read()
